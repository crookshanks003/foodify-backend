...
export class initialDev1644851142673 implements MigrationInterface {
	name = "initialDev1644851142673";
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE TABLE \`order_status\` (\`id\` int NOT NULL AUTO_INCREMENT, \`status\` int NOT NULL DEFAULT '0', \`order_time\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`delivered_time\` datetime NULL, \`dispatch_time\` datetime NULL, \`orderId\` int NULL, UNIQUE INDEX \`REL_014fe4a8ab95c64fdb7b8beb25\` (\`orderId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
		);
		await queryRunner.query(
			`CREATE TABLE \`user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`phone\` varchar(255) NOT NULL, \`is_staff\` tinyint NOT NULL, \`calories\` int NOT NULL DEFAULT '0', \`address\` varchar(255) NULL, \`registered_on\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`CREATE TABLE \`order_item\` (\`id\` int NOT NULL AUTO_INCREMENT, \`quantity\` int NOT NULL, \`orderId\` int NULL, \`foodItemId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`ALTER TABLE \`order_item\` ADD CONSTRAINT \`FK_779432332ca27702001c830fa3f\` FOREIGN KEY (\`foodItemId\`) REFERENCES \`food_item\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
		);
		await queryRunner.query(
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE \`order_item\` DROP FOREIGN KEY \`FK_779432332ca27702001c830fa3f\``,
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`ALTER TABLE \`order_status\` DROP FOREIGN KEY \`FK_014fe4a8ab95c64fdb7b8beb253\``,
		);
		...
		await queryRunner.query(
			`DROP INDEX \`REL_014fe4a8ab95c64fdb7b8beb25\` ON \`order_status\``,
		);
